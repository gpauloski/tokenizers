...
impl From<Error> for neon::result::Throw {
    fn from(err: Error) -> Self {
        let msg = err.0;
        unsafe {
            neon_runtime::error::throw_error_from_utf8(msg.as_ptr(), msg.len() as i64);
            neon::result::Throw
        }
    }
}
...
pub trait Extract {
    fn extract<T: FromJsValue>(&mut self, pos: i64) -> LibResult<T>;
    fn extract_opt<T: FromJsValue>(&mut self, pos: i64) -> LibResult<Option<T>>;
    fn extract_vec<T: FromJsValue>(&mut self, pos: i64) -> LibResult<Vec<T>>;
    fn extract_vec_opt<T: FromJsValue>(&mut self, pos: i64) -> LibResult<Option<Vec<T>>>;
}
impl<'c, T: neon::object::This> Extract for CallContext<'c, T> {
    fn extract<E: FromJsValue>(&mut self, pos: i64) -> LibResult<E> {
        let val = self
            .argument_opt(pos)
    ...
    fn extract_opt<E: FromJsValue>(&mut self, pos: i64) -> LibResult<Option<E>> {
    ...
    fn extract_vec<E: FromJsValue>(&mut self, pos: i64) -> LibResult<Vec<E>> {
        let vec = self
            .argument_opt(pos)
            .ok_or_else(|| Error(format!("Argument {} is missing", pos)))?
            .downcast::<JsArray>()?
    ...
    fn extract_vec_opt<E: FromJsValue>(&mut self, pos: i64) -> LibResult<Option<Vec<E>>> {
